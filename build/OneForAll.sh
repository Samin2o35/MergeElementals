...
window.adManOpenStore = function (url) {
  ...
  function setupMRAID() {
    ...
    if (state === 'loading') {
        ...
        mraid.addEventListener('stateChange', function(newState) {});
        mraid.addEventListener('audioVolumeChange', function(percentage) {});
    }
  }
...
window.adManOpenStore = function (url) 
{
  ...
  var isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) || navigator.userAgent.includes('Macintosh');
  const clickTag = isIOS ? IOS_STORE_URL : ANDROID_STORE_URL;
  if(typeof ScPlayableAd !== 'undefined' && ScPlayableAd.onCTAClick) 
  { 
...
if [[ -f "Mintegral/index.html" ]]; then
  sed -i.bak 's/Cocos Creator | //g' Mintegral/index.html
  rm -f Mintegral/index.html.bak
fi
...
echo "Processing Snapchat..."
mkdir -p Snapchat
cp -r "$FACEBOOK_SRC/"* Snapchat/
if [[ -f "Snapchat/index.html" ]]; then
  target_html="Snapchat/index.html"
  inject_script "$target_html" "$INJECT_SNAPCHAT_SCRIPT"
  touch "Snapchat/config.json"
  config_file="Snapchat/config.json"
  echo "$INJECT_SNAPCHAT_CONFIG" > "$config_file"
  sed -i.bak 's/Cocos Creator | //g' Snapchat/index.html
  rm -f Snapchat/index.html.bak
fi
zip_folder "Snapchat" "Snapchat/Snapchat.zip"
...
# --- APPIER PROCESSING ---
echo "Processing Appier..."
if [[ -d "Unity" ]]; then
    cp -r Unity Appier
    if [[ -f "Appier/Unity.html" ]]; then
        mv Appier/Unity.html Appier/Appier.html
    fi
else
    echo "Warning: Unity folder not found. Cannot copy to Appier."
fi
