      ...
      _export("ThrowMergeGame", ThrowMergeGame = (_dec = ccclass('ThrowMergeGame'), _dec2 = property(_crd && EggDatabase === void 0 ? (_reportPossibleCrUseOfEggDatabase({
        ...
        update(_dt) {
          ...
          for (var i = 0; i < kids.length; i++) {
            ...
            var y = egg.node.worldPosition.y;
        ...
        sortEggDepth() {
          var sorted = this.eggsContainer.children.slice().sort((a, b) => b.worldPosition.y - a.worldPosition.y);

          for (var i = 0; i < sorted.length; i++) sorted[i].setSiblingIndex(i);
        }

        randomStartTier() {
          var max = Math.min(this.maxSpawnTier, this.db.count - 1);
        ...
        popNextEgg() {
          var n = this.nextEggSprite.node;
          var base = this._nextBaseScale;
        ...
        loadLauncher() {
          var tier = this._nextTier; // the shown next egg becomes the docked one

          this._nextTier = this.randomStartTier(); // roll a new next

          this.updateNextDisplay();
          var egg = this.spawnEgg(tier, this.launcher.node.worldPosition);
          if (!egg) return;
          this._dockedEgg = egg;
        ...
        spawnEgg(tier, worldPos) {
          ...
          var node = instantiate(def.prefab);
          var egg = node.getComponent(_crd && Egg === void 0 ? (_reportPossibleCrUseOfEgg({
            error: Error()
          }), Egg) : Egg);
          egg.init({
            tier,
        ...
        spawnMergeFx(x, y, resultTier) {
          ...
          var parent = (_this$fxContainer = this.fxContainer) != null ? _this$fxContainer : this.eggsContainer;
          var node = instantiate(this.mergeEffectPrefab);
          parent.addChild(node);
          node.setWorldPosition(x, y, 0);
          var fx = node.getComponent(_crd && MergeEffect === void 0 ? (_reportPossibleCrUseOfMergeEffect({
            error: Error()
        ...
        playSfx(n, key) {
          ...
          var last = (_this$_lastSfx$key = this._lastSfx[key]) != null ? _this$_lastSfx$key : 0;
        ...
        playAudio(n) {
          ...
          var ac = n.getComponent(_crd && AudioContent === void 0 ? (_reportPossibleCrUseOfAudioContent({
            error: Error()
...
//# sourceMappingURL=9aa1df8f1f8aa2e654029fd2d2b02149c5e0d1b0.js.map